      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 auto-rows-[150px] sm:auto-rows-[200px] md:auto-rows-[220px]">
          {photos.map((photo, index) => (
            <div
              key={index}
              className={`group relative rounded-2xl overflow-hidden shadow-elevation-4 hover:shadow-elevation-6 transition-all duration-500 hover:-translate-y-1 ${photo.span}`}
            >
              <img
                src={photo.src}
                alt={photo.title}
