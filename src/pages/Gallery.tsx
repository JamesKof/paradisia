...
const Gallery = () => {
  ...
  return <main className="min-h-screen bg-background">
      ...
      <section className="py-8 bg-muted/30 border-b border-border">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-3">
            {categories.map(category => <button key={category} onClick={() => setActiveCategory(category)} className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeCategory === category ? "bg-brand-orange text-brand-blue-dark shadow-orange-glow" : "bg-card text-muted-foreground hover:text-brand-orange border border-border hover:border-brand-orange/30"}`}>
                {category}
              </button>)}
          </div>
        </div>
      ...
      <section className="py-16 bg-background">
        <div className="max-w-7xl mx-auto px-4">
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
            {filteredImages.map((image, index) => <div key={image.id} className="group relative break-inside-avoid cursor-pointer overflow-hidden rounded-2xl shadow-elevation-3 hover:shadow-elevation-5 transition-all duration-500" onClick={() => openLightbox(index)}>
          ...
          {filteredImages.length === 0 && <div className="text-center py-16">
      ...
      <Dialog open={selectedImage !== null} onOpenChange={closeLightbox}>
        <DialogContent className="max-w-[95vw] max-h-[95vh] p-0 bg-brand-blue-dark/95 backdrop-blur-xl border-brand-blue overflow-hidden" onKeyDown={handleKeyDown}>
          {selectedImage !== null && <div className="relative flex items-center justify-center min-h-[60vh]">
              {/* Close Button */}
              <button onClick={closeLightbox} className="absolute top-4 right-4 z-50 w-12 h-12 rounded-full bg-brand-blue/80 text-brand-sky-light hover:bg-brand-orange hover:text-brand-blue-dark transition-colors flex items-center justify-center">
                <X className="w-6 h-6" />
              </button>

              {/* Previous Button */}
              <button onClick={goToPrevious} className="absolute left-4 z-50 w-12 h-12 rounded-full bg-brand-blue/80 text-brand-sky-light hover:bg-brand-orange hover:text-brand-blue-dark transition-colors flex items-center justify-center">
              ...
              {/* Next Button */}
              <button onClick={goToNext} className="absolute right-4 z-50 w-12 h-12 rounded-full bg-brand-blue/80 text-brand-sky-light hover:bg-brand-orange hover:text-brand-blue-dark transition-colors flex items-center justify-center">
